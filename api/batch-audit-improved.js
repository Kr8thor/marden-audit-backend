// Improved Batch URL SEO Audit implementation
...
/**
 * Analyze SEO for a specific URL with improved error handling
...
async function analyzeSingleUrl(urlString) {
  ...
  console.log(`[${new Date().toISOString()}] Analyzing SEO for ${normalizedUrl}`);
  
  // Create minimal default result structure for error cases
  const minimalResult = {
    url: normalizedUrl,
  ...
  try {
    ...
    // Fetch the page with appropriate error handling and increased timeout
    let response;
    try {
      ...
      console.log(`Successfully fetched ${normalizedUrl} (${response.status})`);
    } catch (error) {
      ...
      console.error(`Error fetching ${normalizedUrl}: ${errorMessage}`);
    ...
    // Load content into cheerio with a safety check
    let $;
    try {
      $ = cheerio.load(response.data);
    } catch (error) {
      console.error(`Error parsing HTML for ${normalizedUrl}: ${error.message}`);
      return {
        ...minimalResult,
        error: {
          type: 'parsing_error',
          message: 'Failed to parse the page HTML',
          originalError: error.message
        }
      };
    }
    ...
    console.log(`Completed analysis for ${normalizedUrl}`);
    return result;
  } catch (error) {
    console.error(`Unexpected error analyzing SEO for ${normalizedUrl}:`, error);
...
/**
 * Handle batch SEO analysis with improved error handling and timeouts
 * @param {Object} req Express request
 * @param {Object} res Express response
 */
async function handleBatchAudit(req, res) {
  console.log(`[${new Date().toISOString()}] Batch audit request received`);
  ...
  try {
    ...
    // Reduce concurrency limit from 5 to 3 to avoid overwhelming servers
    ...
    for (let i = 0; i < uncachedUrls.length; i += concurrencyLimit) {
      ...
      console.log(`Processing chunk ${Math.floor(i/concurrencyLimit) + 1} of ${Math.ceil(uncachedUrls.length/concurrencyLimit)}: ${chunk.join(', ')}`);
      
      // Process this chunk in parallel with robust error handling
      ...
      try {
        ...
        console.log(`Completed chunk ${Math.floor(i/concurrencyLimit) + 1}, processed ${batchResults.length} URLs so far`);
      } catch (error) {
        console.error(`Error processing chunk:`, error);
        // Add error results for this chunk
        for (const url of chunk) {
          if (!batchResults.some(result => result.url === url)) {
            batchResults.push({
              url,
              score: 0,
              status: 'error',
              error: {
                ...
                message: 'Failed to process URL in batch',
                details: error.toString()
              },
              timestamp: new Date().toISOString(),
              analyzedAt: new Date().toISOString()
    ...
    console.log(`Batch analysis complete for ${normalizedUrls.length} URLs`);
    res.end(JSON.stringify(response));
  } catch (error) {
    ...
    if (!res.headersSent) {
      ...
      // Otherwise, end the response with an error object
